...
execute store result storage mm:solo_scoreboard score2 int 1 run scoreboard players get @s solo_round
loot replace block 0 -64 0 container.0 loot mm:my_head
data modify storage mm:solo_scoreboard username2 set from block 0 -64 0 Items[0].components.minecraft:profile.name
scoreboard players operation $2 solo_leaderboard = @s solo_round
